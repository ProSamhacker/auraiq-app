...
// --- Chat Component ---
const ChatInterface: FC<{ user: User; auth: Auth; db: Firestore }> = ({
  user,
  auth,
  db,
}) => {
  ...
  const handleSelectChat = (chatId: string) => setCurrentChatId(chatId);
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    try {
            ...
            try {
              ...
              if (delta) {
                ...
                const uniqueMessages = Array.from(
                  new Map(
                    [...existingMessages, userMessage, streamingAIMessage].map(
                      (m) => [m.id, m]
                    )
                  ).values()
                );
      ...
      const uniqueMessages = Array.from(
        new Map(
          [...existingMessages, userMessage, finalAIMessage].map((m) => [
            m.id,
            m,
          ])
        ).values()
      ...
      await setDoc(
        chatRef,
        { messages: uniqueMessages, timestamp: Date.now() },
        { merge: true }
      );
    } catch (error) {
      ...
      if (tempChatId) {
        ...
        const uniqueMessages = Array.from(
          new Map([...messages, errorMessage].map((m) => [m.id, m])).values()
        );
        await setDoc(chatRef, { messages: uniqueMessages }, { merge: true });
      }
    } finally {
      setIsLoading(false);
    }
          ...
          {chats.map((chat) => (
            ...
            >
              ...
              <button
                ...
                className="p-1 rounded-md text-gray-500 hover:text-red-400 hover:bg-gray-600/50 opacity-50 hover:opacity-100 transition-all"
              >
        ...
        <div className="p-6 bg-gray-900 border-t border-gray-700">
          <form onSubmit={handleSendMessage} className="relative">
            <textarea
              ...
              placeholder="Type your message to AuraIQ... (Shift+Enter for new line)"
            ...
            <button
              type="submit"
              className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
      ...
      <div className="w-1/4 bg-gray-800 flex flex-col border-l border-gray-700">
        <div className="p-4 border-b border-gray-700">
          ...
          <p className="text-sm text-gray-500">
            Provide persistent context for the AI.
          </p>
        </div>
